...
public class RevivePlayerNearTeamMatesGameRule extends EventGameRule<Boolean> {
	...
	public RevivePlayerNearTeamMatesGameRule(IGame game) {
		super(game, "revivePlayerNearTeamMate", false, ERuleCode.GAME_RULE__REVIVE_PLAYER_NEAR_TEAM_MATES__EXPLANATION);
	}

	@EventHandler(priority = EventPriority.HIGHEST, ignoreCancelled = true)
	private void onPlayerRespawn(PlayerRespawnEvent event) {
		if (!isEnable() || getGame().getState() == PausableState.NOT_STARTED || !getValue() || !(getGame() instanceof ITeamConfigurable))
			return;
